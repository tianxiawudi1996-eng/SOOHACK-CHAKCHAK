...
| `python scripts/harness/audit_gate4_2d_pet_motion.py` | 1 | 자동 QA `PASS`; 상태·reduced-motion 8/8, 수동 승인 0/1로 예상 차단 |
...
Gate 4 감사기의 종료 코드 1은 실패가 아니라 제품 책임자 수동 승인 0/1을 나타내는 통제된 외부 차단이다. 8개 상태, 지속시간, 승인 포즈 매핑, `transform`·`opacity` 제한, reduced-motion 8/8, 랜딩·검토 화면 연결은 모두 통과했다.
