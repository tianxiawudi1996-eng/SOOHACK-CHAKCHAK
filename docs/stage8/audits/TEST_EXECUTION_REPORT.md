...
Gate 4 감사기의 종료 코드 1은 실패가 아니라 제품 책임자 수동 승인 0/1을 나타내는 통제된 외부 차단이다. 8개 상태, 지속시간, 승인 포즈 매핑, `transform`·`opacity` 제한, 120ms 준비·320ms 교차·240ms 착지, 상태 잔동작, 캐릭터 리듬 차이, 빠른 입력 취소, reduced-motion 8/8, 랜딩·검토 화면 연결은 모두 통과했다. 실제 Edge 회귀 검사는 CLS 0과 360·768·1440px 최소 경계 여유 12.11px 이상을 확인했다.
