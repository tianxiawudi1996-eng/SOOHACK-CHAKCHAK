...
| Gate 5~8 | `NOT_STARTED` | Gate 5 진입 허용, AI Behavior부터 순차 구현 필요 |
...
### 우선순위 1: Gate 5 AI Behavior 연결

- 문제 제시, 힌트, 정답, 재시도, 칭찬 이벤트를 캐릭터 상태와 말풍선에 연결한다.
- 잘못된 상태 중복, 무한 반복, 과도한 움직임을 방지한다.
- 상태 우선순위·최소 유지시간·취소 정책·말풍선 중복 제거를 자동 검증한다.
- 제품 책임자 1인 검토 전에는 Gate 6 진입을 허용하지 않는다.
...
랜딩페이지 캐릭터 배치와 Gate 2·3·4 승인은 완료됐다. 전체 Stage 8은 완료되지 않았으며, 다음 실행 작업은 Gate 5 AI Behavior 메타프롬프트 작성과 학습 이벤트·캐릭터 상태·말풍선 연결 구현이다.
