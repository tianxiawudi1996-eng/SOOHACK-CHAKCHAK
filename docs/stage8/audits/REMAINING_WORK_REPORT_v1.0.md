...
| Gate 3 개별 포즈 | `BLOCKED` | PNG 16/16, WebP 16/16 자동 QA PASS, 수동 승인 0/1 |
| 랜딩페이지 캐릭터 배치 | `PROTOTYPE_COMPLETE` | 첫 화면·대화 아바타·성장 리포트에 공식 2D 자산 연결 |
| Gate 4~8 | `NOT_STARTED` | 선행 Gate 승인과 실제 제품 통합 필요 |

### 최종 완료 기준

1. Gate 3 수동 검토 8/8과 제품 책임자 승인 1/1
2. 학습 상태 8개에 대한 포즈·전환 모션·말풍선 이벤트 연결
3. `prefers-reduced-motion` 정적 대체 동작
4. 실제 제품 자산 경로와 컴포넌트 연결
5. 모바일·태블릿·데스크톱 기능·시각·접근성 QA
6. 배포 환경 검증, 모니터링, 롤백 근거 확보
...
- 수동 승인 전에는 Gate 3를 `VERIFIED`로 바꾸거나 Gate 4 진입을 허용하지 않는다.
...
- 검토 URL에서 투명 경계, 정체성, 포즈 누락, 크기·기준선, 소품·색종이, 작은 UI 가독성, PNG/WebP 동등성, 녹색 잔여물을 확인한다.
- 성공: 8/8 확인 후 `APPROVE`
- 실패: 수정이 필요한 포즈와 항목을 특정해 재생성 또는 알파 보정

### 우선순위 2: Gate 4 2D 펫 모션
...
랜딩페이지 캐릭터 배치는 완료됐지만 전체 Stage 8은 완료되지 않았다. 즉시 필요한 다음 결정은 Gate 3 검토 화면의 16개 투명 포즈에 대한 제품 책임자 승인이다. 승인 전에는 Gate 4를 시작하지 않는다.
