...
const expectedNextFields = ['internal_reference', 'evidence_sha256', 'status', 'verified_at'];
const historicalTargetFields = ['target_reference', 'provider_code', 'environment_code', 'connection_reference'];

const phaseIds = Object.keys(status.phases || {}).map(Number).sort((a, b) => a - b);
if (phaseIds.length !== 77 || phaseIds.some((id, index) => id !== index)) failures.push('STATUS_PHASE_0_76_INCOMPLETE');
if (status.current_phase !== 76 || status.phases?.['76']?.status !== 'LOCAL_SOCIAL_LOGIN_LIFECYCLE_PASS_EXTERNAL_PROVIDER_HOLD') failures.push('CURRENT_PHASE_INVALID');
if (status.workflow_state !== 'IN_PROGRESS' || status.local_completion_scope !== 'PHASE_76_LOCAL_SOCIAL_LOGIN_PASS_EXTERNAL_PROVIDER_HOLD' || status.release_state !== 'BLOCKED_EXTERNAL') failures.push('PRODUCT_STATUS_SCOPE_INVALID');
const roadmapIds = [...roadmap.matchAll(/^\| (\d+) \| `/gm)].map((match) => Number(match[1]));
if (roadmapIds.length !== 77 || roadmapIds.some((id, index) => id !== index)) failures.push('ROADMAP_PHASE_0_76_INCOMPLETE');
...
if (nextInput?.type !== 'D80_10_CONTROL_EVIDENCE_REFERENCE' || nextInput?.workstream_id !== 'D80-10' || JSON.stringify(nextInput?.required_fields) !== JSON.stringify(expectedNextFields) || nextInput?.accepted_evidence_count !== 0) failures.push('EXTERNAL_NEXT_INPUT_INVALID');
...
if (routeEvidence.next_required_input?.type !== 'EXTERNAL_DEPLOYMENT_TARGET_REFERENCE' || JSON.stringify(routeEvidence.next_required_input?.fields) !== JSON.stringify(historicalTargetFields)) failures.push('EXTERNAL_ROUTE_EVIDENCE_DRIFT');
...
if (!readme.includes('Phase 76 소셜 로그인 생애주기 로컬 PASS') || !readme.includes('외부 출시 `BLOCKED_EXTERNAL`')) failures.push('README_PRODUCT_SCOPE_INVALID');

console.log(`STATUS_ALIGNMENT_${failures.length ? 'FAIL' : 'PASS'}`);
console.log(`roadmap_phases=${roadmapIds.length}/77 product_phase=${status.current_phase} product_release=${status.release_state}`);
