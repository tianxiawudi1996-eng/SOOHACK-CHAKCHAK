...
const productShell = read('client/design/product-shell.css');
const motionCss = read('assets/stage8/2d-pet-motion-v1.0.css');
const js = read('client/mock/app.js');
const preferredLocale = read('client/i18n/preferred-locale.mjs');
...
if (!html.includes('../design/product-shell.css') || !productShell.includes('--tap-target: 2.75rem') || !productShell.includes('min-height: var(--tap-target)') || !productShell.includes('min-width: var(--tap-target)')) fail('shared product shell or 44px target contract missing');
for (const surface of ['client/diagnostic/index.html','client/math-learning/index.html','client/curriculum/index.html']) {
  const surfaceHtml = read(surface);
  if (!surfaceHtml.includes('class="product-page ') || !surfaceHtml.includes('../design/product-shell.css')) fail(`shared product shell missing from ${surface}`);
}
if (!html.includes('/assets/stage8/2d-pet-motion-v1.0.css')) fail('approved character state motion is not connected to the landing runtime');
if ((html.match(/data-pet-character=/g) ?? []).length !== 2 || (html.match(/data-pet-image(?=\s|>)/g) ?? []).length !== 2 || (html.match(/data-pet-image-stage/g) ?? []).length !== 2) fail('landing character motion roots incomplete');
if (!motionCss.includes('prefers-reduced-motion: reduce') || !motionCss.includes('[data-pet-state="GUIDE"]') || !motionCss.includes('[data-pet-state="PRAISE_PROGRESS"]')) fail('approved character state motion accessibility contract missing');
if (!html.includes('data-i18n="preview.sample"') || !html.includes('id="duet"') || !html.includes('data-i18n="duet.flow.verify"')) fail('sample-data disclosure or dual-tutor differentiation missing');
if (
  !js.includes("const fallback = 'en'") ||
  !js.includes('readPreferredLocale()') ||
  !js.includes('navigator.language') ||
  !preferredLocale.includes("const STORAGE_KEY = 'mathchakchak.locale'") ||
  !preferredLocale.includes('localStorage?.getItem') ||
  !preferredLocale.includes('localStorage?.setItem')
) fail('locale resolver policy incomplete');
...
const imageSources = [...html.matchAll(/<img\b[^>]*\bsrc="([^"]+)"/g)].map((match) => match[1].replace(/^\.\.\/\.\.\//, ''));
