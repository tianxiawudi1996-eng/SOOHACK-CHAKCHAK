...
try {
  ...
  const lockedButEmpty = error?.code === 'EPERM'
    && fs.existsSync(artifactRoot)
    && !fs.readdirSync(artifactRoot, {recursive:true, withFileTypes:true}).some((entry) => entry.isFile());
...
let html = read('client/mock/index.html')
  ...
  .replace('href="app.js?v=', 'href="assets/app.js?v=')
...
const academyHtml = read('client/academy/index.html')
  .replace('../design/tokens.css', '../assets/tokens.css')
  .replace('../design/product-shell.css', '../assets/product-shell.css')
  .replaceAll('../mock/index.html', '../index.html')
  .replace('../../outputs/019fcaf2-285c-7dc3-897a-3c9a2903aac4/2d-pet/v1.0/poses/webp/Chakchaki/chakchaki_p03_guide_v1.0.webp', '../assets/characters/chakchaki-guide.webp')
  .replace('../../outputs/019fcaf2-285c-7dc3-897a-3c9a2903aac4/2d-pet/v1.0/poses/webp/Gongsickyi/gongsickyi_p05_praise-progress_v1.0.webp', '../assets/characters/gongsickyi-praise.webp');
fs.writeFileSync(path.join(site, 'academy', 'index.html'), academyHtml, 'utf8');
for (const file of ['app.js','messages.mjs','styles.css']) fs.copyFileSync(path.join(root, 'client/academy', file), path.join(site, 'academy', file));

const operationsHtml = read('client/operations/index.html')
  .replace('../design/tokens.css', '../assets/tokens.css')
  .replace('../design/product-shell.css', '../assets/product-shell.css')
  .replaceAll('../mock/index.html', '../index.html');
fs.writeFileSync(path.join(site, 'operations', 'index.html'), operationsHtml, 'utf8');
for (const file of ['app.js','messages.mjs','styles.css']) fs.copyFileSync(path.join(root, 'client/operations', file), path.join(site, 'operations', file));
...
fs.copyFileSync(path.join(root, 'client/i18n/preferred-locale.mjs'), path.join(site, 'i18n/preferred-locale.mjs'));
fs.copyFileSync(path.join(root, 'client/accessibility/interaction.mjs'), path.join(site, 'accessibility/interaction.mjs'));
fs.copyFileSync(path.join(root, 'assets/stage8/2d-pet-motion-v1.0.css'), path.join(site, 'assets/stage8/2d-pet-motion-v1.0.css'));
fs.copyFileSync(path.join(root, 'assets/stage8/ai-behavior-v1.0.css'), path.join(site, 'assets/stage8/ai-behavior-v1.0.css'));
const motionRuntime = read('assets/stage8/2d-pet-motion-v1.0.js').replace(
  "const ASSET_ROOT = '/outputs/019fcaf2-285c-7dc3-897a-3c9a2903aac4/2d-pet/v1.0/poses';",
  "const ASSET_ROOT = '/assets/stage8/poses';",
);
fs.writeFileSync(path.join(site, 'assets/stage8/2d-pet-motion-v1.0.js'), motionRuntime, 'utf8');
fs.copyFileSync(path.join(root, 'assets/stage8/ai-behavior-v1.0.js'), path.join(site, 'assets/stage8/ai-behavior-v1.0.js'));
for (const format of ['webp', 'png']) {
  const source = path.join(root, 'outputs/019fcaf2-285c-7dc3-897a-3c9a2903aac4/2d-pet/v1.0/poses', format);
  const destination = path.join(site, 'assets/stage8/poses', format);
...
const manifest = {
  ...
  approved_character_assets:2, approved_pose_assets:32, files, file_count:files.length,
