...
import {evaluateSolutionRecognition,SOLUTION_SOURCE_TYPES} from '../learning/solution-recognition.mjs';
...
const ACADEMY_TRACKS=new Set(['CONCEPT_RECOVERY','SCHOOL_EXAM','ADVANCED_REASONING','CONTEST_BRIDGE']);
...
function route(method, pathname) {
  ...
  if (method === 'GET' && pathname === '/api/v1/admin/math-expert-review/readiness') return {name:'getMathExpertReviewReadiness'};
  if (method === 'GET' && pathname === '/api/v1/admin/ai-tutor-evidence/readiness') return {name:'getTutorExternalEvidenceReadiness'};
  if (method === 'GET' && pathname === '/api/v1/admin/daechi-field-pilot/readiness') return {name:'getDaechiFieldPilotReadiness'};
  if (method === 'GET' && pathname === '/api/v1/admin/commercial-operations/readiness') return {name:'getCommercialOperationsReadiness'};
  if (method === 'POST' && pathname === '/api/v1/local-demo/session') return {name: 'localDemoSession'};
  if (method === 'POST' && pathname === '/api/v1/local-demo/teacher/session') return {name:'localDemoLearningOperatorSession',role:'TEACHER'};
  if (method === 'POST' && pathname === '/api/v1/local-demo/parent/session') return {name:'localDemoLearningOperatorSession',role:'PARENT'};
  ...
  operationsMatch=pathname.match(new RegExp(`^/api/v1/privacy-operations/reference-target-validation-contracts/(${UUID_PATTERN})/reference-proof-handoff-contracts$`));
  ...
  operationsMatch=pathname.match(new RegExp(`^/api/v1/privacy-operations/reference-proof-intake-contracts/(${UUID_PATTERN})/quarantine-readiness-contracts$`));
  ...
  match = pathname.match(new RegExp(`^/api/v1/formula-lessons/(${UUID_PATTERN})/responses/(${UUID_PATTERN})/tutor-feedback$`));
  if (method === 'POST' && match) return {name:'createTutorFeedback',formulaSessionId:match[1],responseId:match[2]};
  ...
  match = pathname.match(new RegExp(`^/api/v1/students/(${UUID_PATTERN})/academy-readiness$`));
  if (method === 'GET' && match) return {name:'getAcademyReadiness',studentId:match[1]};
  match = pathname.match(new RegExp(`^/api/v1/students/(${UUID_PATTERN})/academy-curriculum$`));
  if (method === 'GET' && match) return {name:'getAcademyCurriculum',studentId:match[1]};
  match = pathname.match(new RegExp(`^/api/v1/operations/students/(${UUID_PATTERN})/overview$`));
  if(method==='GET'&&match)return {name:'getLearningOperationsOverview',studentId:match[1]};
  match = pathname.match(new RegExp(`^/api/v1/operations/students/(${UUID_PATTERN})/assignments$`));
  if(method==='POST'&&match)return {name:'createAcademyLearningAssignment',studentId:match[1]};
  match = pathname.match(new RegExp(`^/api/v1/operations/students/(${UUID_PATTERN})/assignments/(${UUID_PATTERN})/transition$`));
  if(method==='POST'&&match)return {name:'transitionAcademyLearningAssignment',studentId:match[1],assignmentId:match[2]};
  match = pathname.match(new RegExp(`^/api/v1/operations/students/(${UUID_PATTERN})/interventions$`));
  if(method==='POST'&&match)return {name:'createStudentLearningIntervention',studentId:match[1]};
  match = pathname.match(new RegExp(`^/api/v1/students/(${UUID_PATTERN})/solution-recognition/evaluations$`));
  if (method === 'POST' && match) return {name:'createSolutionRecognitionEvaluation',studentId:match[1]};
  return null;
}

async function execute(repository, request, match, requestId, {auth, metrics, tutorService, tutorOperations}) {
  ...
  if(match.name==='localDemoLearningOperatorSession'){
    if(!auth.localDemoEnabled)throw notFound();
    const context=await repository.createLocalDemoLearningOperator({role:match.role});
    return success({access_token:createSessionToken({userId:context.userId,role:context.role},auth.sessionSecret),token_type:'Bearer',expires_in:300,role:context.role,student_id:context.studentId,environment:'LOCAL_SYNTHETIC_ONLY'},{requestId,status:201});
  }
  ...
  if(match.name==='getAcademyReadiness'){
    const url=new URL(request.url,'http://localhost');
    const targetTrack=String(url.searchParams.get('target')||'ADVANCED_REASONING').toUpperCase();
    if(!ACADEMY_TRACKS.has(targetTrack))throw badRequest('INVALID_ACADEMY_TRACK','error.invalid_academy_track');
    const data=await repository.getAcademyReadiness({actor,studentId:requireUuid(match.studentId,'student_id'),targetTrack});
    return success(data,{requestId});
  }
  if(match.name==='getAcademyCurriculum'){
    const url=new URL(request.url,'http://localhost');
    const targetTrack=String(url.searchParams.get('target')||'ADVANCED_REASONING').toUpperCase();
    if(!ACADEMY_TRACKS.has(targetTrack))throw badRequest('INVALID_ACADEMY_TRACK','error.invalid_academy_track');
    const requestedLocale=normalizeRequestedLocale(url.searchParams.get('locale')||'ko');
    const data=await repository.getAcademyCurriculum({actor,studentId:requireUuid(match.studentId,'student_id'),targetTrack,requestedLocale});
    return success(data,{requestId,locale:requestedLocale});
  }
  if(match.name==='getAcademyCurriculumReadiness'){
    const data=await repository.getAcademyCurriculumReadiness({actor});
    return success(data,{requestId});
  }
  if(match.name==='getLearningOperationsOverview'){
    const data=await repository.getLearningOperationsOverview({actor,studentId:requireUuid(match.studentId,'student_id')});
    return success(data,{requestId});
  }
  if(match.name==='getContentCorpusReadiness'){
    const data=await repository.getContentCorpusReadiness({actor});
    return success(data,{requestId});
  }
  if(match.name==='getLearningEffectPilotReadiness'){
    const data=await repository.getLearningEffectPilotReadiness({actor});
    return success(data,{requestId});
  }
  if(match.name==='getMathExpertReviewReadiness'){
    const data=await repository.getMathExpertReviewReadiness({actor});
    return success(data,{requestId});
  }
  if(match.name==='getTutorExternalEvidenceReadiness'){
    const data=await repository.getTutorExternalEvidenceReadiness({actor});
    return success(data,{requestId});
  }
  if(match.name==='getDaechiFieldPilotReadiness'){
    const data=await repository.getDaechiFieldPilotReadiness({actor});
    return success(data,{requestId});
  }
  if(match.name==='getCommercialOperationsReadiness'){
    const data=await repository.getCommercialOperationsReadiness({actor});
    return success(data,{requestId});
  }
  ...
  if(match.name==='getExternalReferenceProofHandoffContract'){
  ...
  if(match.name==='getExternalReferenceProofQuarantineReadinessContract'){
  ...
  if(match.name==='createSolutionRecognitionEvaluation'){
    const sourceType=String(body.source_type??'').toUpperCase();
    if(!SOLUTION_SOURCE_TYPES.includes(sourceType))throw badRequest('INVALID_SOLUTION_SOURCE_TYPE','error.invalid_solution_source_type');
    const studentId=requireUuid(match.studentId,'student_id');
    let candidates=[];
    let providerReference='OCR_PROVIDER_NOT_CONFIGURED';
    let providerVerified=false;
    let imageQuality=0;
    let mathConsistency=0;
    let stepContinuity=0;
    if(sourceType==='MANUAL_TEXT'){
      if(typeof body.expression!=='string'||body.expression.trim().length<1||body.expression.length>500){
        throw badRequest('INVALID_MANUAL_EXPRESSION','error.invalid_manual_expression');
      }
      candidates=[{expression:body.expression,confidence:1}];
      providerReference='MANUAL_TEXT_LOCAL_V1';
      mathConsistency=1;
      stepContinuity=1;
    }
    const evaluation=evaluateSolutionRecognition({
      source_type:sourceType,candidates,provider_verified:providerVerified,
      image_quality:imageQuality,math_consistency:mathConsistency,step_continuity:stepContinuity
    });
    const data=await repository.createSolutionRecognitionEvaluation({
      actor,studentId,providerReference,providerVerified,evaluation,key,hash,requestId
    });
    return success(data,{requestId,status:data.replayed?200:201,extraMeta:{replayed:data.replayed}});
  }
  if(match.name==='createAcademyLearningAssignment'){
    const dueAt=new Date(body.due_at);
    const now=Date.now();
    if(!Number.isFinite(dueAt.getTime())||dueAt.getTime()<=now||dueAt.getTime()>now+90*86400000)throw badRequest('INVALID_ASSIGNMENT_DUE_AT','error.invalid_assignment_due_at');
    const data=await repository.createAcademyLearningAssignment({actor,studentId:requireUuid(match.studentId,'student_id'),curriculumId:requireUuid(body.curriculum_id,'curriculum_id'),dueAt:dueAt.toISOString(),key,hash,requestId});
    return success(data,{requestId,status:data.replayed?200:201,extraMeta:{replayed:data.replayed}});
  }
  if(match.name==='transitionAcademyLearningAssignment'){
    const toStatus=String(body.to_status||'').toUpperCase();
    if(!['IN_PROGRESS','COMPLETED','CANCELLED'].includes(toStatus))throw badRequest('INVALID_ASSIGNMENT_STATUS','error.invalid_assignment_status');
    const data=await repository.transitionAcademyLearningAssignment({actor,studentId:requireUuid(match.studentId,'student_id'),assignmentId:requireUuid(match.assignmentId,'assignment_id'),toStatus,key,hash,requestId});
    return success(data,{requestId,extraMeta:{replayed:data.replayed}});
  }
  if(match.name==='createStudentLearningIntervention'){
    const reasonCode=String(body.reason_code||'').toUpperCase(),priority=String(body.priority||'').toUpperCase(),actionCode=String(body.action_code||'').toUpperCase();
    if(!['BASELINE_MISSING','CONCEPT_GAP','RECALL_GAP','APPLICATION_GAP','INDEPENDENCE_GAP','TEACHER_REVIEW_REQUIRED'].includes(reasonCode))throw badRequest('INVALID_INTERVENTION_REASON','error.invalid_intervention_reason');
    if(!['NORMAL','WATCH','URGENT'].includes(priority))throw badRequest('INVALID_INTERVENTION_PRIORITY','error.invalid_intervention_priority');
    if(!['COLLECT_EVIDENCE','REBUILD_CONCEPT','SCHEDULE_RECALL','PRACTICE_TRANSFER','REVIEW_SOLUTION'].includes(actionCode))throw badRequest('INVALID_INTERVENTION_ACTION','error.invalid_intervention_action');
    const data=await repository.createStudentLearningIntervention({actor,studentId:requireUuid(match.studentId,'student_id'),assignmentId:body.assignment_id?requireUuid(body.assignment_id,'assignment_id'):null,reasonCode,priority,actionCode,key,hash,requestId});
    return success(data,{requestId,status:data.replayed?200:201,extraMeta:{replayed:data.replayed}});
  }
  ...
  if(match.name==='createTutorFeedback'){
    if(!tutorService)throw new ApiError(503,'TUTOR_SERVICE_UNAVAILABLE','error.tutor_service_unavailable');
    const data=await tutorService.respond({
      actor,formulaSessionId:requireUuid(match.formulaSessionId,'formula_session_id'),
      responseId:requireUuid(match.responseId,'response_id'),key,hash
    });
    return success(data,{requestId,status:data.replayed?200:201,extraMeta:{replayed:data.replayed}});
  }
...
export function createMathChakChakServer({repository, auth, tutorService=null, tutorOperations=null, metrics = new OperationsMetrics()}) {
